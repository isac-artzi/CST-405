...
/*#4*/
/*#todo newLabel(): hand out a fresh jump target — "L0", "L1", ...  Every*/
/*#todo loop, branch and switch needs its own, and they must never repeat.*/
char* newLabel(void) {
 ...
 */
/*#4*/
/*#todo The break-label stack.  When you start generating a loop you push the*/
/*#todo loop's exit label; when you finish you pop it.  A `break` anywhere in*/
/*#todo between just jumps to whatever is on top.  That one stack is the whole*/
/*#todo implementation of `break`, including the nested case.*/
...
static int generateArgTAC(ASTNode* arg);  /*#3*/

/*#3*/
/*#todo generateArgTAC: walk the argument list, emit code for each argument*/
/*#todo expression and then an ARG instruction for it, and return the count.*/
/*#todo The count goes into the CALL instruction so the back end knows how*/
/*#todo many of the buffered ARGs belong to this call — which is what makes*/
/*#todo nested calls like f(1, g(2)) come out right.*/
...
/*#4*/
/*#todo Map the six relational operator codes to their TAC opcodes.*/
...
/*#5*/
/*#todo Map the logical operator codes to TAC_AND, TAC_OR and TAC_NOT.*/
...
/*#3*/
/*#todo Generate TAC for a NODE_FUNC_CALL and a NODE_ARRAY_INDEX.*/
/*#todo */
/*#todo A call becomes:   ARG a1 ; ARG a2 ; t = CALL f, 2*/
/*#todo An index becomes: t = arr[i]     (a single ARRAY_LOAD)*/
/*#todo */
/*#todo Both return the NAME of the temporary holding the result, because*/
/*#todo that is the contract generateTACExpr has with its callers.*/
        case NODE_FUNC_CALL: {
...
/*#end*/
/*#3*/
/*#todo Assignment now has two shapes.  If arrayLHS is set, evaluate the*/
/*#todo index and emit ARRAY_STORE; otherwise emit a plain ASSIGN.*/
...
/*#3*/
/*#todo Generate TAC for a NODE_RETURN: evaluate the expression (if any) and*/
/*#todo emit RETURN.  Emitting RETURN is easy; making it actually leave the*/
/*#todo function is codegen.c's job, and it is the part people forget.*/
        case NODE_RETURN: {
...
/*#end*/
/*#5*/
/*#todo Lower a NODE_IF to labels and jumps.  There is no `if` in TAC; the*/
/*#todo shape you want is:*/
/*#todo      t = <condition>*/
/*#todo      IF_FALSE t GOTO Lelse*/
/*#todo      <then branch>*/
/*#todo      GOTO Lend*/
/*#todo   Lelse:*/
/*#todo      <else branch>*/
/*#todo   Lend:*/
/*#todo Handle the no-else case by jumping straight to Lend.*/
/*#todo Every label you need comes from newLabel().*/
        case NODE_IF: {
            /* Generate labels */
...
/*#end*/
/*#4*/
/*#todo Lower NODE_WHILE and NODE_FOR to labels and jumps.*/
/*#todo */
/*#todo   while (c) B          for (init; c; upd) B*/
/*#todo   ───────────          ────────────────────*/
/*#todo   Lstart:              <init>*/
/*#todo     t = c              Lstart:*/
/*#todo     IF_FALSE t -> Lend   t = c*/
/*#todo     B                    IF_FALSE t -> Lend*/
/*#todo     GOTO Lstart          B*/
/*#todo   Lend:                  <upd>*/
/*#todo                          GOTO Lstart*/
/*#todo                        Lend:*/
/*#todo */
/*#todo Notice that a for-loop is a while-loop with two extra pieces —*/
/*#todo which is why many compilers desugar `for` into `while` and reuse*/
/*#todo the code below.  Either approach is fine; say which you chose.*/
/*#todo */
/*#todo Push Lend on the break-label stack around the BODY so that a*/
/*#todo `break` inside knows where to jump, and pop it afterwards.*/
        case NODE_WHILE: {
            /* Generate labels */
...
/*#end*/
/*#3*/
/*#todo NODE_BLOCK just recurses into its statement list — the scope it*/
/*#todo opened mattered to semantic analysis and is finished with by now.*/
/*#todo NODE_FUNC_CALL as a statement evaluates and discards the result.*/
        case NODE_BLOCK:
...
/*#3*/
/*#todo NODE_ARRAY_DECL: emit ARRAY_DECL for a real array (which reserves*/
/*#todo size*4 bytes) but PARAM for an array parameter (which receives an*/
/*#todo address).  One node type, two very different meanings.*/
...
/*#end*/
/*#5*/
/*#todo Lower NODE_SWITCH.  The shape that makes fall-through fall out for*/
/*#todo free is a DISPATCH section followed by a BODY section:*/
/*#todo */
/*#todo    DECL __swN ; __swN = <controlling expr>   (evaluated ONCE)*/
/*#todo    t = __swN == c0 ; IF_FALSE t -> next ; GOTO Lbody0*/
/*#todo    ... one test per case ...*/
/*#todo    GOTO Ldefault (or Lend when there is no default)*/
/*#todo  Lbody0: <body of case 0>      <- no jump at the end:*/
/*#todo  Lbody1: <body of case 1>         control falls into the next body,*/
/*#todo  Lend:                            which IS fall-through*/
/*#todo */
/*#todo `break` becomes GOTO Lend, so push Lend on the break-label stack*/
/*#todo before generating the bodies.*/
/*#todo A jump TABLE is the classic alternative — faster, but only when the*/
/*#todo case values are dense.  Discuss the trade-off in your write-up.*/
        case NODE_SWITCH: {
...
/*#end*/
/*#4*/
/*#todo Lower NODE_BREAK: emit GOTO to the label on top of the break-label*/
/*#todo stack.  If the stack is empty the semantic analyzer should already*/
/*#todo have rejected the program, so this is a belt-and-braces check.*/
        case NODE_BREAK: {
...
/*#3*/
/*#todo generateParamTAC / generateTACFuncDef / generateTACList.*/
/*#todo A function definition becomes:*/
/*#todo     FUNC_BEGIN f ; PARAM int a ; ... body ... ; FUNC_END f*/
/*#todo Mark an array parameter with the type "int[]" so the code generator*/
/*#todo knows the slot holds an address rather than the data.*/
...
/*#end*/
/*#3*/
/*#todo The program is now a list of declarations and function definitions,*/
/*#todo so walk it with generateTACList instead of wrapping it in an*/
/*#todo implicit main.  Every function gets its own FUNC_BEGIN/FUNC_END pair.*/
...
/*#4*/
/*#todo BRANCH SIMPLIFICATION.  Once constant folding has run, a condition*/
/*#todo may be a literal:  IF_FALSE 0 GOTO L  always jumps, and*/
/*#todo IF_FALSE 1 GOTO L  never does.  Rewrite the first as GOTO L and*/
/*#todo delete the second.  Deleting it is what makes the code AFTER it*/
/*#todo unreachable, which is what lets the next pass remove that too —*/
/*#todo a small demonstration of why passes must run to a fixed point.*/
