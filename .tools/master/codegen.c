...
static int paramIndex = 0;        /* Which $aN the next parameter arrives in */  /*#3*/
...
/*#3*/
/*#todo Array addressing.  Three cases, and telling them apart IS the lesson:*/
/*#todo   global array      la   $t, g_name          address of a .data label*/
/*#todo   local array       addi $t, $sp, offset     the elements are in the frame*/
/*#todo   array parameter   lw   $t, offset($sp)     the slot holds someone*/
/*#todo                                              ELSE'S base address*/
/*#todo Then element address = base + index*4  (sll by 2, then add).*/
...
/*#4*/
/*#todo MIPS has a "set on condition" instruction for each comparison, and*/
/*#todo SPIM supplies three-operand pseudo-instructions for all six:*/
/*#todo   slt  sgt  sle  sge  seq  sne*/
/*#todo Each writes 1 or 0 into the destination register, which is exactly*/
/*#todo the 1/0 our language says a comparison produces.*/
...
/*#3*/
/*#todo Parameters arrive in $a0-$a3.  The prologue's job is to copy each one*/
/*#todo into its frame slot, because $a0-$a3 will be needed again the moment*/
/*#todo this function calls something else.*/
            case TAC_PARAM: {
                Symbol* s = lookupSymbol(i->result);
...
/*#4*/
/*#todo The relational opcodes emit exactly like the arithmetic ones —*/
/*#todo two operands in, one register out — so just add their case labels*/
/*#todo to this group.  Six labels, no new code.*/
...
/*#5*/
/*#todo LOGICAL OPERATORS.  The trap here is that MIPS `and` and `or` are*/
/*#todo BITWISE:  2 and 1  is 0, but in C  2 && 1  is 1.  So normalise each*/
/*#todo operand to exactly 0 or 1 first ( sne $t, $operand, $zero ) and*/
/*#todo only then apply the bitwise instruction.*/
/*#todo   !x  is just  seq $d, $x, $zero.*/
            /* ---------- logical operators --------------------------------
...
/*#4*/
/*#todo CONTROL FLOW.  Four TAC opcodes become four MIPS instructions:*/
/*#todo   LABEL L      ->   L:*/
/*#todo   GOTO L       ->   j    L*/
/*#todo   IF_FALSE t L ->   beq  $t, $zero, L*/
/*#todo   IF_TRUE  t L ->   bne  $t, $zero, L*/
/*#todo*/
/*#todo The subtle part is the register cache.  Two different paths can reach*/
/*#todo the same label with different values cached in $t0-$t9, so at every*/
/*#todo label and every branch you must write back what is dirty and then*/
/*#todo forget everything.  Skip that and your loops will produce beautiful,*/
/*#todo wrong answers.  Call flushRegisters() — and think about WHERE in the*/
/*#todo branch cases it has to go relative to reading the condition register.*/
            /* ---------- control flow -------------------------------------
...
/*#3*/
/*#todo CALLS AND RETURNS — the heart of the calling convention.*/
/*#todo */
/*#todo For a CALL:*/
/*#todo   1. put the arguments in $a0-$a3 while the $t registers are still valid*/
/*#todo   2. THEN flush every dirty register to memory ($t0-$t9 are*/
/*#todo      caller-saved: the callee may clobber all of them)*/
/*#todo   3. jal to the function label*/
/*#todo   4. the result comes back in $v0*/
/*#todo Getting steps 1 and 2 in the wrong order is the classic bug, and it*/
/*#todo produces a compiler that works until a function takes two arguments.*/
/*#todo */
/*#todo For a RETURN: put the value in $v0, then JUMP TO THE EPILOGUE.  Do not*/
/*#todo just fall through — a `return` in the middle of a function has to skip*/
/*#todo everything after it, and the epilogue is the only place that restores*/
/*#todo $ra and pops the frame.*/
/*#todo */
/*#todo An array passed as an argument is passed as its base ADDRESS.*/
            /* ---------- calls --------------------------------------------- */
            case TAC_ARG:
...
/*#end*/
/*#3*/
/*#todo ARRAY_LOAD and ARRAY_STORE.  Compute the element address once with*/
/*#todo elementAddrReg, then a single lw or sw.  Note ARRAY_STORE keeps the*/
/*#todo value being stored in the `result` field — read the TAC listing rather*/
/*#todo than guessing which field is which.*/
            /* ---------- arrays -------------------------------------------- */
