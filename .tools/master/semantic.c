...
/*#3*/
/*#todo A second table, for FUNCTIONS.  Variables and functions live in separate*/
/*#todo namespaces, which is why a program may declare both `int total;` and*/
/*#todo `int total(...)`.  Record just enough to check a call: the name, the*/
/*#todo parameter count, and which parameters are arrays.*/
...
static FunctionSymbol functions[MAX_FUNCTIONS];  /*#3*/
static int functionCount = 0;                    /*#3*/
static Scope scopes[MAX_SCOPE_DEPTH];
static int scopeDepth = 0;
static char* currentFunction = NULL;  /*#3*/
static int inFunction = 0;            /*#3*/
/*#4*/
/*#todo A counter for how many loops (Topic 5: and switches) enclose the*/
/*#todo statement being checked.  `break` is legal only when it is > 0.*/
...
void initSemantic() {
    ...
    breakDepth = 0;  /*#4*/
/*#3*/
/*#todo Reset the function table too, and seed it with the one function the*/
/*#todo language provides for free: `print`, which takes one argument.*/
/*#todo Registering it here is what makes `print(x)` type-check like any*/
/*#todo other call instead of needing a special case.*/
    functionCount   = 0;
...
/*#3*/
/*#todo Arrays need their own scope-insertion helper — not because the storage*/
/*#todo question is different (symtab.c settles that later) but because the*/
/*#todo error messages are.*/
...
/*#3*/
/*#todo findFunction / addFunction / countParams / countArgs.*/
/*#todo countParams walks the parameter list parser.y built; countArgs walks*/
/*#todo the argument list at a call site.  Comparing those two numbers is the*/
/*#todo whole of call checking.*/
/* Function management */
...
/*#3*/
/*#todo Check a NODE_FUNC_CALL: does the function exist, and does the call*/
/*#todo supply the right number of arguments?  Then check each argument*/
/*#todo expression.  A wrong argument count is the single most common mistake*/
/*#todo a compiler can catch for the programmer here.*/
        case NODE_FUNC_CALL: {
...
/*#end*/
/*#3*/
/*#todo Check a NODE_ARRAY_INDEX: the array must be declared, and the index is*/
/*#todo an expression that must itself check out.  Note what we do NOT check —*/
/*#todo whether the index is in range.  That is not knowable at compile time in*/
/*#todo general, and this language has no run-time check either.  Say so in*/
/*#todo your write-up; it is a real design decision, not an oversight.*/
...
/*#end*/
/*#3*/
/*#todo The target of an assignment may now be an array ELEMENT.  Check*/
/*#todo the index expression in that case, and the variable name in the*/
/*#todo other.*/
...
/*#3*/
/*#todo Check a NODE_RETURN.  The interesting case is `return` OUTSIDE any*/
/*#todo function — legal to parse, meaningless to translate.  That gap between*/
/*#todo "parses" and "means something" is the territory this phase owns.*/
        case NODE_RETURN:
            if (!inFunction) {
                fprintf(stderr, "\n╔════════════════════════════════════════════════════════════╗\n");
...
/*#end*/
/*#5*/
/*#todo Check a NODE_IF: the condition is an expression, the branches are*/
/*#todo statements.  Nothing about `if` needs a new kind of check — it just*/
/*#todo needs all three children visited, which is the point: analysis*/
/*#todo recurses over structure, and new structure means new recursion.*/
        case NODE_IF:
...
/*#end*/
/*#4*/
/*#todo Check NODE_WHILE and NODE_FOR.  Two things matter here:*/
/*#todo   • visit every child (condition, body, and for-loop init/update)*/
/*#todo   • bracket the BODY with breakDepth++ / breakDepth-- so that a*/
/*#todo     `break` inside it is recognised as legal*/
/*#todo Note the condition is NOT inside the bracket: `break` in a loop*/
/*#todo condition would make no sense.*/
        case NODE_WHILE:
...
/*#3*/
/*#todo NODE_BLOCK opens a scope and closes it again.  Two lines of code, and*/
/*#todo they are what make this legal:*/
/*#todo     int x;  { int x; }      inner x shadows outer x*/
/*#todo and this an error:*/
/*#todo     { int x; int x; }       same name twice in one scope*/
/*#todo Handle NODE_FUNC_CALL used as a statement and NODE_STMT_LIST too.*/
        case NODE_BLOCK:
            enterScope();
...
/*#5*/
/*#todo Check a NODE_SWITCH.  Beyond visiting the children, a switch has*/
/*#todo two rules of its own worth enforcing:*/
/*#todo   • at most ONE default clause*/
/*#todo   • no two case labels with the same constant value*/
/*#todo Both are errors a student compiler can catch cheaply and a*/
/*#todo programmer is genuinely glad to have caught.*/
/*#todo Remember breakDepth++/-- around the clause bodies.*/
        case NODE_SWITCH: {
...
/*#end*/
/*#4*/
/*#todo Check NODE_BREAK: it is legal only when breakDepth > 0, i.e. only*/
/*#todo inside a loop (Topic 5 adds switch to that list).  This is the*/
/*#todo smallest possible example of a CONTEXT-SENSITIVE rule — no grammar*/
/*#todo can express it, which is exactly why semantic analysis exists.*/
        case NODE_BREAK:
            if (breakDepth == 0) {
                fprintf(stderr, "\n╔════════════════════════════════════════════════════════════╗\n");
...
/*#end*/
/*#3*/
/*#todo Check a NODE_ARRAY_DECL.  Two forms to tell apart: a real declaration*/
/*#todo (which needs a positive size) and a parameter (which has no size at*/
/*#todo all).  Reject `int a[0];` and `int a[-3];` here — nothing downstream*/
/*#todo will.*/
...
/*#3*/
/*#todo addParamsToScope, checkFuncDef, registerFunction, registerFunctions*/
/*#todo and checkFunctions — the TWO-PASS structure.*/
/*#todo */
/*#todo Pass 1 records every function signature.  Pass 2 checks the bodies.*/
/*#todo Two passes and not one, so that a function may call another defined*/
/*#todo LATER in the file.  Collapse them into one pass and see which programs*/
/*#todo stop compiling; that experiment is worth five minutes.*/
...
/*#end*/
/*#3*/
/*#todo Replace the single walk with TWO passes:*/
/*#todo   pass 1: registerFunctions(root) — record every signature*/
/*#todo   pass 2: checkFunctions(root)    — check every body*/
